...
def back_to_selected_rides():
    ...
    return redirect('/selected-rides')
...
def booked_rides():
    ...
    # Assuming each ride document has a 'passenger_id' and 'rating' field
    rides_with_ratings = []
    for ride in rides:
        rider = users_collection.find_one({'username': ride['passenger_name']})
        if rider and 'feedback' in rider and len(rider['feedback']) > 0:
            # Assuming 'rating' is a field in the 'feedback' array
            ratings = [feedback['rating'] for feedback in rider['feedback']]
            average_rating = sum(ratings) / len(ratings)
            ...
            ride['rating'] = 0
        rides_with_ratings.append(ride)
    return render_template('booked-rides.html', rides=rides_with_ratings, data=data)
...
def delete_ride_rider():
    ...
    query = {'rider_id': rider_id, 'start_point': start_point, 'end_point': end_point, 'start_time': start_time, 'date': date}
    
    print("Query for Deletion:")
    print(query)
    
    # Check if the document exists before deletion
    selectrides_document = selectrides_collection.find(query)
    postride_document = postride_collection.find(query)
    
    print("Document in selectrides_collection:")
    print(selectrides_document)
    print("Document in postride_collection:")
    print(postride_document)
    
    selectrides_result = selectrides_collection.delete_many(query)
    postride_result = postride_collection.delete_many(query)
    
    print(f"Deleted from selectrides_collection: {selectrides_result.deleted_count}")
    print(f"Deleted from postride_collection: {postride_result.deleted_count}")
...
@app.route('/final/<username>', methods=['GET'])
def final(username):
    return render_template('final.html', username=username)


@app.route('/ride-action', methods=['POST'])
def ride_action():
    if 'user_id' in session:
        user_id = session['user_id']
        user = users_collection.find_one({'_id': ObjectId(user_id)})
    ...
    print("Received Values:")
    print(f"Rider ID: {rider_id} (Type: {type(rider_id)})")
    print(f"Passenger ID: {passenger_id} (Type: {type(passenger_id)})")
    print(f"Start Point: {start_point} (Type: {type(start_point)})")
    print(f"End Point: {end_point} (Type: {type(end_point)})")
    print(f"Start Time: {start_time} (Type: {type(start_time)})")
    print(f"Date: {date} (Type: {type(date)})")
    print(f"Button Pressed: {button_pressed} (Type: {type(button_pressed)})")

    if button_pressed == 'map':
        return show_passenger_map(rider_id, data)
    if button_pressed == 'cancel':
        return cancel_ride_passenger(data, rider_id, passenger_id, start_point, end_point, start_time, date)

def show_passenger_map(rider_id, data):
    passenger_id = session.get('user_id')
    ...
    passenger = selectrides_collection.find_one({'rider_id': rider_id})
    if not passenger or 'end_point' not in passenger:
        return jsonify({'error': 'Passenger end point not found'}), 404
...
def cancel_ride_passenger(data, rider_id, passenger_id, start_point, end_point, start_time, date):
    ...
    rider = postride_collection.find_one({'rider_id': rider_id, 'start_point': start_point, 'end_point': end_point, 'start_time': start_time, 'date': date})
    if not rider or 'vacant_seats' not in rider:
        return jsonify({'error': 'Rider or vacant seats not found'}), 404

    vacant_seats = rider['vacant_seats']
    vacant_seats += int(request.form.get('booked_seats', 0))
    
    postride_collection.update_one(
        {'rider_id': rider_id, 'start_time': start_time, 'end_point': end_point, 'start_point': start_point, 'date': date},
        {'$set': {'vacant_seats': vacant_seats}}
    )
    return redirect('/selected-rides')
